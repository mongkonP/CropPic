...
namespace TORServices.Forms
{
    public partial class MyProgressBar : ProgressBar
    {
        ...
        public void AddValue(int increment = 1)
        {
            if (increment <= 0) return; // ป้องกันค่าที่ไม่ถูกต้อง
            this.Invoke(new Action(() =>
            {
                this.Value = Math.Min(this.Value + increment, this.Maximum);
                this.Update();
            }));
        }

        public void SetMinMax(int min = 0, int max = 100,int v = 0)
        {
            this.Invoke(new Action(() =>
            {
                this.Minimum = min < 0 ? 0 : min; // ตรวจสอบค่าต่ำสุด
                this.Maximum = max > 0 ? max : 1; // ตรวจสอบค่าสูงสุด
                this.Value = v; // กำหนดค่าเริ่มต้นเป็นค่าต่ำสุด
            }));
        }
